	...
	*/
	...
	this.renderTile = function(i, l){
		...
		var index = i,
			...
			tile = 			_.get(_this.tiles, "["+l+"]["+index+"]") || {swatch: _this.layers[l].defaultSwatch},
			cachedTile = 	_.get(TILE_CACHE, "["+l+"]["+index+"]"),
		...
		setTimeout(function () {
			if(
				...
				if(!TILE_CACHE[l]) TILE_CACHE[l] = []
				TILE_CACHE[l][index] = tile

				if(RENDER_CACHE[l]) ctx.putImageData(RENDER_CACHE[l], 0, 0)

				if(_.get(swatch, 'image')) {
					ctx.drawImage(swatch.image, world[0], world[1])
					console.log('->',swatch)
				} else {
					console.log('swatch not found')
					// return console.log(swatch)
				}
	...
	*/
	...
	this.drawLayers = function(){
		for(var l = 0; l < _this.layers.length; l++) {
			if(RENDER_CACHE[l]) {
				_this.ctx.putImageData( RENDER_CACHE[l], 0, 0)
			} else {
	...
	this.getCache = function() {
		return [RENDER_CACHE, TILE_CACHE]
	}
